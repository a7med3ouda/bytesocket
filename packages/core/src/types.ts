 ...
 */
export interface ISocketRooms<TInstance, TEvents extends SocketEvents> {
	/**
	 ...
	 */
	publishRaw: (room: string, message: ServerOutgoingData, isBinary?: boolean, compress?: boolean) => TInstance;
	/**
 ...
 * @typeParam SD - The socket data type.
 */
export interface IServerLifecycle<TServer, TEvents extends SocketEvents, SD extends SocketData> {
	/** Register a listener for the HTTP upgrade phase. */
	onUpgrade: <CB extends (...args: unknown[]) => void>(callback: CB) => TServer;
	/** Remove a listener for the HTTP upgrade phase. */
	offUpgrade: <CB extends (...args: unknown[]) => void>(callback?: CB) => TServer;
	/** Register a one-time listener for the HTTP upgrade phase. */
	onceUpgrade: <CB extends (...args: unknown[]) => void>(callback: CB) => TServer;

	/** Register a listener for socket open (after successful auth). */
	onOpen: (callback: (socket: ISocket<TEvents, SD>) => void) => TServer;
	/** Remove a listener for socket open. */
	offOpen: (callback?: (socket: ISocket<TEvents, SD>) => void) => TServer;
	/** Register a one-time listener for socket open. */
	onceOpen: (callback: (socket: ISocket<TEvents, SD>) => void) => TServer;

	/** Register a listener for authentication success. */
	onAuthSuccess: (callback: (socket: ISocket<TEvents, SD>) => void) => TServer;
	/** Remove a listener for authentication success. */
	offAuthSuccess: (callback?: (socket: ISocket<TEvents, SD>) => void) => TServer;
	/** Register a one-time listener for authentication success. */
	onceAuthSuccess: (callback: (socket: ISocket<TEvents, SD>) => void) => TServer;

	/** Register a listener for authentication failure. */
	onAuthError: (callback: (socket: ISocket<TEvents, SD>, ctx: ErrorContext) => void) => TServer;
	/** Remove a listener for authentication failure. */
	offAuthError: (callback?: (socket: ISocket<TEvents, SD>, ctx: ErrorContext) => void) => TServer;
	/** Register a one-time listener for authentication failure. */
	onceAuthError: (callback: (socket: ISocket<TEvents, SD>, ctx: ErrorContext) => void) => TServer;

	/** Register a listener for raw incoming messages. */
	onMessage: (callback: (socket: ISocket<TEvents, SD>, data: ServerIncomingData, isBinary: boolean) => void) => TServer;
	/** Remove a listener for raw incoming messages. */
	offMessage: (callback?: (socket: ISocket<TEvents, SD>, data: ServerIncomingData, isBinary: boolean) => void) => TServer;
	/** Register a one-time listener for raw incoming messages. */
	onceMessage: (callback: (socket: ISocket<TEvents, SD>, data: ServerIncomingData, isBinary: boolean) => void) => TServer;

	/** Register a listener for socket close. */
	onClose: (callback: (socket: ISocket<TEvents, SD>, code: number, reason: ServerIncomingData) => void) => TServer;
	/** Remove a listener for socket close. */
	offClose: (callback?: (socket: ISocket<TEvents, SD>, code: number, reason: ServerIncomingData) => void) => TServer;
	/** Register a one-time listener for socket close. */
	onceClose: (callback: (socket: ISocket<TEvents, SD>, code: number, reason: ServerIncomingData) => void) => TServer;

	/** Register a listener for errors. */
	onError: (callback: (socket: ISocket<TEvents, SD> | null, ctx: ErrorContext) => void) => TServer;
	/** Remove a listener for errors. */
	offError: (callback?: (socket: ISocket<TEvents, SD> | null, ctx: ErrorContext) => void) => TServer;
	/** Register a one-time listener for errors. */
	onceError: (callback: (socket: ISocket<TEvents, SD> | null, ctx: ErrorContext) => void) => TServer;
}
 ...
 * @typeParam SD - The socket data type.
 */
export type IServerRooms<TServer, TEvents extends SocketEvents, SD extends SocketData> = {
	/**
	 ...
	 */
	publishRaw: (room: string, message: ServerOutgoingData, isBinary?: boolean, compress?: boolean) => TServer;
	/**
	 * Emit a typed event to a specific room (server-side publish).
	 *
	 * @typeParam R - Room name (must be a key in `TEvents['emitRoom']`).
	...
	lifecycle: {
		 ...
		 * });
		 */
		onJoin: (callback: (socket: ISocket<TEvents, SD>, room: string, next: MiddlewareNext) => void) => TServer;
		/** Remove a guard for single room join requests. */
		offJoin: (callback?: (socket: ISocket<TEvents, SD>, room: string, next: MiddlewareNext) => void) => TServer;
		/** Register a one-time guard for single room join requests. */
		onceJoin: (callback: (socket: ISocket<TEvents, SD>, room: string, next: MiddlewareNext) => void) => TServer;

		/** Register a guard for single room leave requests. */
		onLeave: (callback: (socket: ISocket<TEvents, SD>, room: string, next: MiddlewareNext) => void) => TServer;
		/** Remove a guard for single room leave requests. */
		offLeave: (callback?: (socket: ISocket<TEvents, SD>, room: string, next: MiddlewareNext) => void) => TServer;
		/** Register a one-time guard for single room leave requests. */
		onceLeave: (callback: (socket: ISocket<TEvents, SD>, room: string, next: MiddlewareNext) => void) => TServer;
	};
	/** Bulk operations for multiple rooms. */
	bulk: {
		/**
		...
		lifecycle: {
			...
			onJoin: (callback: (socket: ISocket<TEvents, SD>, rooms: string[], next: MiddlewareNext) => void) => TServer;
			/** Remove a guard for bulk room join requests. */
			offJoin: (callback?: (socket: ISocket<TEvents, SD>, rooms: string[], next: MiddlewareNext) => void) => TServer;
			/** Register a one-time guard for bulk room join requests. */
			onceJoin: (callback: (socket: ISocket<TEvents, SD>, rooms: string[], next: MiddlewareNext) => void) => TServer;

			/** Register a guard for bulk room leave requests. */
			onLeave: (callback: (socket: ISocket<TEvents, SD>, rooms: string[], next: MiddlewareNext) => void) => TServer;
			/** Remove a guard for bulk room leave requests. */
			offLeave: (callback?: (socket: ISocket<TEvents, SD>, rooms: string[], next: MiddlewareNext) => void) => TServer;
			/** Register a one-time guard for bulk room leave requests. */
			onceLeave: (callback: (socket: ISocket<TEvents, SD>, rooms: string[], next: MiddlewareNext) => void) => TServer;
		};
	};
};
 ...
 * });
 */
export interface ByteSocketOptionsBase<TEvents extends SocketEvents = SocketEvents, SD extends SocketData = SocketData> {
...
export interface IByteSocket<TEvents extends SocketEvents = SocketEvents, SD extends SocketData = SocketData> {
	 ...
	 *
	 * - Closes **all** active WebSocket connections gracefully (code `1001`).
	 * - Invokes the transport‑specific cleanup (e.g., shuts down the underlying
	 *   WebSocket server, removes HTTP upgrade listeners, releases native
	 *   uWebSockets.js routes).
	 * - Clears every registered event listener, global/room middleware,
	 *   lifecycle hook, and any internal state (callback maps, middleware arrays,
	 *   room subscriptions).
	 * - After calling `destroy()`, the instance **cannot be reused**.
	 *
	 * If you attached the instance to an external HTTP server (Node.js `http.Server`),
	 * you may need to call `server.removeAllListeners("upgrade")` after `destroy`
	 * if you intend to use a new ByteSocket instance on the same server.
	 *
	 * @example
	 * ```ts
	 * const io = new ByteSocket();
	 ...
	 * io.destroy();
	 *
	 * // (Optional) free the HTTP server for a new instance:
	 * // server.removeAllListeners("upgrade");
	 * // const io2 = new ByteSocket();
	 * // io2.attach(server, "/ws");
	 * ```
	 */
	destroy(): void;
	/**
