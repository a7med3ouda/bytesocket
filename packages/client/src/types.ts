...
import type { MsgpackrOptions } from "@bytesocket/core";

/**
 * The data types actually received from listening to messages.
 * (Excluding Blob to keep queue handling simple.)
 */
export type ClientIncomingData = string | ArrayBuffer;
/**
 * The data types actually accepted by the browser WebSocket.send() method.
 * (Excluding Blob to keep queue handling simple.)
 */
export type ClientOutgoingData = string | BufferSource;
...
export type ByteSocketOptions = {
	...
	/** Interval between ping messages in ms. @default 50000 */
	pingInterval?: number;
	/** Time to wait for pong response before closing connection. @default 40000 */
	pingTimeout?: number;
	/** Authentication configuration. */
	auth?: AuthConfig;
